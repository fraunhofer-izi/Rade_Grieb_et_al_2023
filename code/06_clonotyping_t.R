...
se.meta = readRDS(paste0(manifest$meta$work, "integration/seurat_harmony_t_pub.Rds"))

# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
se.meta = subset(se.meta, subset = STUDY == "Grieb et al.")
...
fltrd.vdj = fltrd.vdj[grepl("vdj_t/", fltrd.vdj)]
...
saveRDS(se.meta, paste0(manifest$meta$work, "integration/seurat_harmony_grieb_clonotypes.Rds"))
saveRDS(combined, paste0(manifest$meta$work, "integration/scRepertoire_combined.Rds"))
